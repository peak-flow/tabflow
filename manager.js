...
class WindowManager {
  ...
  async closeDomainTabs(domain, tabIds) {
    ...
    try {
      ...
      this.showToast(`Closed ${tabIds.length} ${domain} tab${tabIds.length !== 1 ? 's' : ''}`, 'success');
      
    } catch (error) {
      console.error('Failed to close domain tabs:', error);
      this.showToast(`Failed to close ${domain} tabs`, 'error');
    }
  }
